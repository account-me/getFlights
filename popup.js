  ...
  $(document).ready(function() {
	...
	chrome.storage.local.get(['flights', 'outbound', 'inbound', 'amman'], function(result) {
	  ...
	  function getThrid(startDate, endDate){
		...
		var getTh3 = [sortedFlights[0],sortedFlights[1]]
		return getTh3;
	  }
		  ...
		  flInOut.forEach(function(ee){
			  ...
			  }else
			  {
				...
				try{
					...
					if(thisThrid[0].depart1.includes("المدينة")){var deThrid = thisThrid[0].depart1.replace("المنورة ", "");}else{var deThrid = thisThrid[0].depart1;}
					if(thisThrid[0].arrive1.includes("(+1)")){var arThrid = thisThrid[0].arrive1.replace("(+1)", "");}else{var arThrid = thisThrid[0].arrive1;}
					var newProducts1 = '<div class="flight-box '+ ee.site +'">';arThrid
					var tot13 = thisThrid[0].amount1 + thisThrid[0].amount1;
					if(ee.timeout == 1){
					...
					newProducts1       += '<div class="col-2"><bdi>' + thisThrid[0].amount1 + ' <div class="rs1">RS</div></bdi></div>';
					newProducts1       += '<div class="col-2 total-rs">اجمالي بالريال</div>';
					newProducts1    += '</div>';
					newProducts1     += '<div class="row">';
					newProducts1       += '<div class="col-4 seat2">' + thisThrid[0].date1 + vv2 +'</div>';
			  	...
			  	try{
					...
					var thisThrid = getThrid(ee.dateNum1, ee.dateNum2);
					var total99 = ee.total + thisThrid[0].amount1;
					if(thisThrid[0].depart1.includes("المدينة")){var deThrid = thisThrid[0].depart1.replace("المنورة ", "");}else{var deThrid = thisThrid[0].depart1;}
					if(thisThrid[0].arrive1.includes("(+1)")){var arThrid = thisThrid[0].arrive1.replace("(+1)", "");}else{var arThrid = thisThrid[0].arrive1;}
					if(ee.timeout == 1){
					...
					newProducts3    += '</div>';
					newProducts3     += '<div class="row">';
					newProducts3       += '<div class="col-4 seat2">' + thisThrid[0].date1 + vv2 +'</div>';
					newProducts3       += '<div class="col-2"><bdi>' + deThrid + '</bdi></div>';
					newProducts3       += '<div class="col-2"><bdi>' + arThrid + '</bdi></div>';
					newProducts3       += '<div class="col-2"><bdi>' + thisThrid[0].amount1 + ' <div class="rs1">RS</div></bdi></div>';
